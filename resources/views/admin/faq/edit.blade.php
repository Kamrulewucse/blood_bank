    ...
    <div class="row">
        ...
        <div class="col-md-12">
            ...
            <div class="card card-outline card-primary">
                ...
                <!-- /.card-header -->
                <!-- form start -->
                <form action="{{ route('faq.edit',['faq'=>$faq->id]) }}" enctype="multipart/form-data" class="form-horizontal" method="post">
                    @csrf
                    <div class="card-body">
                        ...
                        <div class="form-group row {{ $errors->has('question') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-10">
                                <textarea rows="3"  name="question" class="form-control" id="question" placeholder="Enter Short answer">{{ old('question',$faq->question) }}</textarea>
                                @error('question')
                                <span class="help-block">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <div class="form-group row {{ $errors->has('answer') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-10">
                                <textarea rows="3"  name="answer" class="form-control" id="answer" placeholder="Enter answer">{{ old('asnwer',$faq->answer) }}</textarea>
                                @error('answer')
                                <span class="help-block">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <div class="form-group row {{ $errors->has('status') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-10">
                                <div class="icheck-success d-inline">
                                    <input checked type="radio" id="active" name="status" value="1" {{ empty(old('status')) ? ($errors->has('status') ? '' : ($faq->status == '1' ? 'checked' : '')) :
                                ...
                                <div class="icheck-danger d-inline">
                                    <input type="radio" id="inactive" name="status" value="0" {{ empty(old('status')) ? ($errors->has('status') ? '' : ($faq->status == '0' ? 'checked' : '')) :
    ...
    <script>
        $(function () {
           ...
           // $('#answer').summernote();
            ...
            var subCategorySelected = '{{ empty(old('sub_category')) ? ($errors->has('sub_category') ? '' : $faq->sub_category_id) : old('sub_category') }}';
            ...
            var subSubCategorySelected = '{{ empty(old('sub_sub_category')) ? ($errors->has('sub_sub_category') ? '' : $faq->sub_sub_category_id) : old('sub_sub_category') }}';
